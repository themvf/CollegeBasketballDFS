...
def load_saved_phantom_review_outputs(
    ...
    store = CbbGcsStore(bucket_name=bucket_name, client=client)
    try:
        summary_payload = store.read_phantom_review_summary_json(selected_date, resolved_run_id)
    except Exception:
        return empty_result
    ...
    summary_df = pd.DataFrame(summary_rows) if isinstance(summary_rows, list) else pd.DataFrame()

    phantom_parts: list[pd.DataFrame] = []
    for version_item in summary_payload.get("version_csv_blobs") or []:
        if not isinstance(version_item, dict):
            continue
        version_key = str(version_item.get("version_key") or "").strip()
        if not version_key:
            continue
        try:
            csv_text = store.read_phantom_review_csv(selected_date, resolved_run_id, version_key)
        except Exception:
            continue
        if not csv_text:
            continue
        try:
            version_df = pd.read_csv(io.StringIO(csv_text))
        except Exception:
            continue
        if version_df.empty:
            continue
        if "version_key" not in version_df.columns:
            version_df["version_key"] = version_key
        phantom_parts.append(version_df)

    phantom_df = pd.concat(phantom_parts, ignore_index=True) if phantom_parts else pd.DataFrame()
    return {
        "summary_payload": summary_payload,
        "summary_df": summary_df,
        "phantom_df": phantom_df,
    }
...
@st.cache_data(ttl=600, show_spinner=False)
def load_resolved_rotowire_slate_context(
    selected_date: date,
    slate_key: str,
    contest_type: str,
    slate_name: str,
...
def build_optimizer_pool_for_date(
    ...
    slate_bundle = load_active_slate_context(
        ...
        slate_name=str(st.session_state.get("load_slate_rotowire_slate_name") or "All"),
        slate_id=(
            _safe_int_value(st.session_state.get("load_slate_rotowire_slate_id"), default=0)
            or None
        ),
...
with tab_dk:
    ...
    if repair_dk_slate_clicked:
        if repair_dk_slate_upload is None:
            st.error("Choose a DraftKings CSV file before running fallback repair.")
        else:
            with st.spinner("Repairing missing DK IDs from uploaded DraftKings CSV..."):
                try:
                    repair_summary = import_dk_slate_overrides(
                        dk_slate_csv_bytes=repair_dk_slate_upload.getvalue(),
                        selected_date=dk_slate_date,
                        slate_key=dk_slate_key,
                        contest_type=rotowire_contest_type,
                        slate_name=(rotowire_slate_name.strip() or "All"),
    ...
    if not bucket_name:
        st.info("Set a GCS bucket in sidebar to load legacy DraftKings slate data.")
    else:
        try:
...
with tab_slate_vegas:
    ...
    if not bucket_name:
        ...
        try:
            pool_df, removed_injured_df, raw_slate_df, _, season_history_df, slate_context = build_optimizer_pool_for_date(
                bucket_name=bucket_name,
...
with tab_lineups:
    ...
    if "save_phantom_outputs_to_gcs" not in st.session_state:
        st.session_state["save_phantom_outputs_to_gcs"] = True
    ...
    if not bucket_name:
        ...
        try:
            pool_df, removed_injured_df, raw_slate_df, _, _, slate_context = build_optimizer_pool_for_date(
                bucket_name=bucket_name,
    ...
    if not bucket_name:
        ...
        try:
            ...
            if not phantom_manifests:
                ...
                phantom_df_state = st.session_state.get("cbb_phantom_review_df")
                phantom_summary_state = st.session_state.get("cbb_phantom_summary_df")
                phantom_meta_state = st.session_state.get("cbb_phantom_review_meta") or {}
                active_phantom_run_id = str(phantom_meta_state.get("run_id") or "").strip()
                active_phantom_date = str(phantom_meta_state.get("date") or "").strip()
                needs_saved_phantom_restore = bool(
                    selected_run_id
                    and (
                        not isinstance(phantom_summary_state, pd.DataFrame)
                        or phantom_summary_state.empty
                        or active_phantom_run_id != selected_run_id
                        or active_phantom_date != tr_date.isoformat()
                    )
                )
                if needs_saved_phantom_restore:
                    restored_bundle = load_saved_phantom_review_outputs(
                        bucket_name=bucket_name,
                        selected_date=tr_date,
                    ...
                    restored_payload = restored_bundle.get("summary_payload")
                    restored_summary_df = restored_bundle.get("summary_df")
                    restored_phantom_df = restored_bundle.get("phantom_df")
                    if not isinstance(restored_payload, dict):
                        restored_payload = {}
                    if not isinstance(restored_summary_df, pd.DataFrame):
                        restored_summary_df = pd.DataFrame()
                    if not isinstance(restored_phantom_df, pd.DataFrame):
                        restored_phantom_df = pd.DataFrame()
                    if not restored_summary_df.empty or not restored_phantom_df.empty:
                        restored_versions = [
                            str(item.get("version_key") or "").strip()
                            for item in (restored_payload.get("version_csv_blobs") or [])
                            if isinstance(item, dict) and str(item.get("version_key") or "").strip()
                        ]
                        st.session_state.pop("cbb_phantom_source_lineups", None)
                        st.session_state.pop("cbb_phantom_source_meta", None)
                        st.session_state["cbb_phantom_review_df"] = restored_phantom_df
                        st.session_state["cbb_phantom_summary_df"] = restored_summary_df
                        st.session_state["cbb_phantom_review_meta"] = {
                            "date": tr_date.isoformat(),
                            "run_id": selected_run_id,
                            "contest_id": str(restored_payload.get("contest_id") or tr_contest_id),
                            "versions": restored_versions,
                            "compared_to_field": bool(restored_payload.get("compared_to_field", False)),
                            "restored_from_gcs": True,
                        }
            ...
            if isinstance(phantom_df_state, pd.DataFrame) and not phantom_df_state.empty:
                if phantom_meta:
                    restored_suffix = " | Source: `GCS restore`" if bool(phantom_meta.get("restored_from_gcs")) else ""
                    st.caption(
